...
class ThreadTestCase(TestCase):
    def setUp(self):
        self.user1 = User.objects.create_user("user1", None, "tst12345")
        self.user2 = User.objects.create_user("user2", None, "tst12345")
        self.user3 = User.objects.create_user("user3", None, "tst12345")
    ...
    def test_thread_filtered_by_users(self):
        self.thread.users.add(self.user1, self.user2)
        threads = Thread.objects.filter(users=self.user1).filter(users=self.user2)
        self.assertEqual(self.thread, threads[0])

    def test_thread_no_exists_filtered_by_users(self):
        threads = Thread.objects.filter(users=self.user1).filter(users=self.user2)
